	...
	route.post(
		...
		async (req: Request, res: Response, next: NextFunction) => {
			try {
				...
				const alreadyExists = await FindOpponent.findOne({
				...
				if (alreadyExists) {
					...
					const ratingInfo = await Record.findOne({
						game_id: matchInfo.game_id,
						user_id: matchInfo.user_id,
					});
					...
					let matchUserId = -1;
					//find wating users
					const waitingUsers = await FindOpponent.find({
						game_id: matchInfo.game_id,
						matched_with: -1,
					});
					for (let i = 0; i < waitingUsers.length; ++i) {
						if (
							Math.abs(waitingUsers[i].rating - rat) < gameInfo.matching_range
						) {
							matchUserId = waitingUsers[i].user_id;
							await FindOpponent.update(
								{ game_id: matchInfo.game_id, user_id: matchUserId },
								{ matched_with: matchInfo.user_id },
							);
							break;
						}
					}
					...
					let matchRecord = FindOpponent.create({
						game_id: matchInfo.game_id,
						user_id: matchInfo.user_id,
					...
					res.status(201).json({
						success: true,
						timeout: gameInfo.matching_duration,
						location: matchInfo.location,
					});
				}
			} catch (e) {
			...
			try {
				...
				if (alreadyExists) {
					if (alreadyExists.matched_with !== -1) {
						const opponent = await FindOpponent.findOne({
							user_id: alreadyExists.matched_with,
							game_id: matchInfo.game_id,
						});
						res.status(200).json({
							success: true,
							...
							opponent_id: alreadyExists.matched_with,
						});
					} else {

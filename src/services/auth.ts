...
export default class AuthService {
	...
	public async SignUp(userInputDTO: IUserInputDTO): Promise<{ user: IUser }> {
		try {
			/* 모든 정보가 입력되었는지 확인한다 */
			if (
				typeof userInputDTO.user_name === 'undefined' ||
				typeof userInputDTO.game_id === 'undefined'
			) {
			...
			let gameRecord = Record.create({
				game_id: userInputDTO.game_id,
				user_id: userRecord.user_id,
				rating_1: config.rating.r1_init,
				rating_2: config.rating.r2_init,
			});

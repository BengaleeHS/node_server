...
export default class RecordService {
	public async Record(dataMatchLog: IDataMatchLog) {
		...
		try {
			...
			let prevRecordA = await Record.findOne({
				game_id: dataMatchLog.game_id,
				user_id: dataMatchLog.user_a_id,
			});
			let prevRecordB = await Record.findOne({
				game_id: dataMatchLog.game_id,
				user_id: dataMatchLog.user_b_id,
			});

			if (
				typeof prevRecordA === 'undefined' ||
				typeof prevRecordB === 'undefined'
			) {
			...
			const [rating_1_a, rating_1_b] = this.rating(
				prevRecordA.rating_1,
				prevRecordB.rating_1,
				dataMatchLog.a_point,
				dataMatchLog.b_point,
				'elo',
				config.rating.r1_k,
			);
			const [rating_2_a, rating_2_b] = this.rating(
				prevRecordA.rating_1,
				prevRecordB.rating_1,
				dataMatchLog.a_point,
				dataMatchLog.b_point,
			...
			await Record.update(
				{ game_id: dataMatchLog.game_id, user_id: dataMatchLog.user_a_id },
				{ rating_1: rating_1_a, rating_2: rating_2_a },
			);
			await Record.update(
				{ game_id: dataMatchLog.game_id, user_id: dataMatchLog.user_b_id },
				{ rating_1: rating_1_b, rating_2: rating_2_b },
			);
	...
	private rating(
		...
		switch (ratingType) {
			...
			case 'decay':
				return [
					this.decay(ratingA, scoreA, parameter),
					this.decay(ratingB, scoreB, parameter),
				];
			default:
				return [-1, -1];
		}
